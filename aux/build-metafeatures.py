...
To score these features, we first need an iterable list of them.  For this, we go back to the user marks themselves.  We go through all the global xmin and xmax ranges, mark by mark, and quantify by measure of closeness of marking midpoint, which chunks of x-range indicate markings which are "alike" enough to be consolidated together (requiring that matching marks have non-zero overlap).  The consolidated markings, which we call metafeatures, are written and saved to a csv.
...
# This edited version of the metafeature list builder DOES NOT allow a single user mark to contribute to multiple metafeatures.
...
tolerance = 0.8      # How close (in days) the midpoints of two user markings must be if indicating the same feature
cutoff = 3.0         # Duration (in days) of the longest user marking we're willing to assume was made intentionally
...
for i in range(len(setsubjectids)):
    ...
    if len(subidclasses)==0:                                                    # If this leaves no classification rows left
        ...
        for j in range(len(subidclasses)):
            ...
            matchidx = [j]                                                      # List of matches includes original mark by default    
            ...
            if duration >= cutoff:                                              # Filter out marks wider than cutoff
                ...
                for k in range(len(subidclasses)):
                    ...
                    if k == j:                                                  # Don't compare a mark to itself
                        ...
                        continue
                    elif testduration >= cutoff:                                # Ignore very wide marks
                        continue
                ...
                if len(matchidx)==1:                                            # If the original index is the only one in the match list
                    ...
                    matchidx_list = [matchidx]                                  # Initialize list of match lists, hold on tight
                    for y in range(len(matchidx)-1):
                        ...
                        for yk in range(len(subidclasses)):
                            ...
                            if yk == idx:                                       # Don't compare a mark to itself
                                continue
                            elif yk in done:                                    # Don't compare to marks already consolidated
                                continue
                        ...
                        matchidx_list.append(matchidx_y)             # Append match list to list of lists
                        
                    # When done with all y indices, we should have a list of "sets,"
                    # some of which may match the original set and some of which may not
                    
                    matchidx_new = [j]                                          # Make new list to store indices of identical lists
                    for jy in range(len(matchidx)-1):
                        jset = set(matchidx)
                        yset = set(matchidx_list[jy+1])
                        if jset==yset:                                          # If the match set for two indices is identical
                            matchidx_new.append(matchidx[jy+1])                 # Add index corresponding to set to the list (NOT the whole set)
                        elif jset!=yset:                                        # Otherwise,
                            continue                                            # Continue
